...
public class JointHelper : MonoBehaviour
{
    ...
    void lockPosition()
    {
        if (LockXPosition || LockYPosition || LockZPosition)
        {
            ...
            transform.localPosition = new Vector3(LockXPosition ? initialPosition.x : currentPosition.x, LockYPosition ? initialPosition.y : currentPosition.y/*, gameObject.transform.position.y*/,LockZPosition ? initialPosition.z : currentPosition.z);
        }
